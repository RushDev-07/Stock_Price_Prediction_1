...
from src.evaluation import evaluate_regression_metrics, evaluate_classification_metrics
...
def main():
    ...
    data = yf.download("KO", start="2014-01-01", interval='1d')
    ...
    #hybrid_model = HybridModel( model_type="RandomForest",
    #n_estimators=300,
    #max_depth=15,
    #min_samples_split=5,
    #min_samples_leaf=2)
    #hybrid_model.train(train_features, train_target)
    hybrid_model = HybridModel(input_size=train_features.shape[1], hidden_size=75, num_layers=2, dropout_rate=0.2, epochs=75, batch_size=32)
    hybrid_model.train(train_features, train_target)
